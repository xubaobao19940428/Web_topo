  ...
  <div class="topo-properties">
    ...
    <template v-if="configObject != null && isLayer == false">
      ...
      <div class="topo-properties-table">
        ...
        <div v-show="tabIndex == 1&&nodeType==0">
          <div class="not-surpport">
            <q-list separator no-border>
             <q-expansion-item label="物模型配置" default-opened>
              <table>
                  <tr>
                    <td width="30%">产品名称</td>
                    <td width="70%">
                      <q-select v-model="wumoxing.name" :options="options"  option-value="identifier" option-label="name" @input="detailDevice"/>
                    </td>
                  </tr>
                  <tr>
                    <td width="40%">最大值</td>
                    <td width="60%">
                      <q-input v-model="wumoxing.max" label="最大值" readonly/>
                    </td>
                  </tr>
                  <tr>
                    <td width="40%">最小值</td>
                    <td width="60%">
                      <q-input v-model="wumoxing.min" label="最小值" readonly/>
                    </td>
                  </tr>
                   <tr>
                    <td width="40%">单位</td>
                    <td width="60%">
                      <q-input v-model="wumoxing.unit" label="单位" readonly/>
                    </td>
                  </tr>
                ...
                <div style="width:100%;padding:10px 10px 10px 10px;">
                      <q-btn color="secondary" label="确定"  style="width:100%" @click="addWumoxing"/>
                  </div>
             </q-expansion-item>
            </q-list>
          </div>
        </div>
        <div v-show="tabIndex == 1&&nodeType==1">
          <div class="not-surpport">
            <q-list separator no-border>
             <q-expansion-item label="网关子产品" default-opened>
              <table>
                  <tr>
                    <td width="30%">名称</td>
                    <td width="70%">
                      <q-select v-model="product" :options="productlist"  option-value="objectId" option-label="name" @input="detailProduct"/>
                    </td>
                  </tr>  
              </table>
               <div style="width:100%;padding:10px 10px 10px 10px;">
                      <q-btn color="secondary" label="绑定产品"  style="width:100%" @click="addProduct"/>
               </div>
             </q-expansion-item>
            </q-list>
          </div>
                  ...
                  <tr>
                    ...
                    <td>
                      <q-select
                        ...
                        :option-disable="(item) => item === null ? true : item.cannotSelect"
                        :options="[{label:'打开链接',value:'link'},{label:'赋值变量',value:'val'},{label:'展示隐藏',value:'visible'},{label:'调用服务',value:'service'},{label:'弹窗',value:'alert'},{label:'拍照取证',value:'image',},{label:'视频取证',value:'video',},{label:'音频取证',value:'audio',},{label:'上一页',value:'last'},{label:'下一页',value:'next'}]"
                      />
                    </td>
                  </tr>
...
export default {
  ...
  methods: {
    ...
    addWumoxing(){
      this.configObject.wumoxing.identifier=this.wumoxing.name.identifier
      this.configObject.wumoxing.max = this.wumoxing.max
      this.configObject.wumoxing.min = this.wumoxing.min
      this.configObject.wumoxing.unit = this.wumoxing.unit
      console.log(this.configObject)
    },
    //网关设备添加productId
    addProduct(){
      this.configObject.productId = this.productId
    },
    //产品设备
    detailDevice(val){
        this.wumoxing.max = val.dataType.specs.max
        this.wumoxing.min = val.dataType.specs.min
        this.wumoxing.unit = val.dataType.specs.unit
    },
    //网关设备
    detailProduct(val){
      console.log(val)
      this.productId = val.objectId
    }
    
  },
  created() {
      setTimeout(()=>{
        if(localStorage.getItem('nodeType')==1){
          this.productlist = JSON.parse(localStorage.getItem('productlist'))
          this.nodeType=1
        }else{
          this.options = JSON.parse(localStorage.getItem('wumoxing'))
          this.nodeType=0
        }
      },1000)
  }
