<template>
<div class="wresize-panel-2" @touchend="onMouseup($event)" @touchmove="onMousemove($event)">
    <template v-if="fixedWidthMode == 'left'">
        <div class="wresize-panel-left" :style="{width: divLeft.width + 'px'}">
            <slot name="left"></slot>
        </div>

        <div class="wresize-handler" @touchstart="onMouseDownLeft($event)" :style="{left: divHandler.left + 'px'}"></div>
    ...
    <template v-else>
        ...
        <div class="wresize-handler" @touchstart="onMouseDownRight($event)" :style="{left: divHandler.right + 'px'}"></div>
